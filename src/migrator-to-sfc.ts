...
const injectScss = (scssSourceFile: SourceFile | undefined, vueTemplate: string): string => {
  if (!scssSourceFile) {
    return vueTemplate;
  }
  //Match the style tag.
...
export const getStyleSrc = (vueSourceFile: SourceFile): string | undefined => {
...
export const vueFileToSFC = async (
  ...
  if (styleFileRelativePath) {
    ...
    vueFileText = injectScss(styleSourceFile, vueFileText);
